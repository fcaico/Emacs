...
<xsl:text>1 ; </xsl:text><xsl:value-of select="normalize-space(title)"/><xsl:text> ; </xsl:text><xsl:choose><xsl:when test="count(section)=0">3</xsl:when><xsl:otherwise>0</xsl:otherwise></xsl:choose><xsl:text> ; ../html/jde-ug/jde-ug-content.html#</xsl:text><xsl:value-of select="title/anchor/@id"/><xsl:text>
</xsl:text>
<xsl:for-each select="section">
