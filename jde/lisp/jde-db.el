...
;; $Revision: 1.77 $ $Date: 2001/07/31 05:11:50 $ 
...
(defcustom jde-db-debugger (cons "jdb" (cons "" "Executable"))
"Specify the debugger you want to use to debug Java programs.  Select
JDEbug, if you want to use the JDE's builtin debugger.  Select jdb, if
...
Otherwise, select Other. Other may be an executable or a Java
class. If Other is a class, the JDE launches a vm to run the
...
;; (makunbound 'jde-db-marker-regexp)
...
(defvar jde-db-oldjdb-marker-regexp
  "^Breakpoint hit: .*(\\([^\$]*\\).*:\\([0-9]*\\))"
...
(defvar jde-db-stack-depth "")
(make-variable-buffer-local 'jde-db-stack-depth)
...
(defun jde-db-marker-filter (input)
  ...
  (let* ((output "")
	 ...
	 (marker-regexp 
	  (if oldjdbp jde-db-oldjdb-marker-regexp jde-db-marker-regexp))
    ...
    (if (string-match marker-regexp jde-db-marker-acc)
    ...
    (if (string-match "^[-a-zA-Z0-9_$]+\\[\\([0-9]+\\)\\] " jde-db-marker-acc)
	(setq jde-db-stack-depth (match-string 1 jde-db-marker-acc)))
    ...
    (if (string-match (concat "^  \\[" jde-db-stack-depth
			      "\\] .*(\\([^\$\n]*\\).*:\\([0-9]+\\))")
		      jde-db-marker-acc)
	(let ((marker (match-string 0 jde-db-marker-acc))
	      (class (match-string 1 jde-db-marker-acc))
	      (line-no (string-to-int (match-string 2 jde-db-marker-acc))))
    ...
    (if (string-match "\\(^Breakpoint hit:\\)\\|\\(^Step completed:\\)" jde-db-marker-acc)
	(progn
	...
	(let* ((debug-buffer (get-buffer-create debug-buf-name))
		...
		(cond
		 ...
		 ((string= debugger "Other")
		  (if (string= (cdr (cdr jde-db-debugger)) "Executable")
