...
for a description of the macro.  If something is already bound to this
key, Emacs will ask you whether you want to override this
binding. Furthermore, this package also takes care of saving the macro
to your .emacs file for later Emacs sessions.
...
you power-macros. You execute this function by pressing `M-x' and
typing `pm-manage-macros'.
 
Changes
=======
Well search for the words History information,
and you will find a detailed description of changes.
 
Home Page
=========
The home page for power macros is http://www.imada.sdu.dk/~blackie/emacs/ 
...
;;
;; When you have loaded this package Emacs will, upon completion of ;; macro definition, ask you which key you want to assign this macro ;; to and ask for a description of the macro. 
;; If something is already bound to this key, Emacs will ask you ;; whether you want to override this binding. Furthermore, this
;; package also takes care of saving the macro to your .emacs file ;; for later Emacs sessions.
;;
;; The description for all the defined macros may be obtained with the ;; function `pm-manage-macros'. Using this function you can also ;; manage you power-macros. You execute this function by pressing `M-x' and ;; typing `pm-manage-macros'.
 
;;}}}
;;{{{ Installation description
 
;; To install this package, copy this file to a directory in your ;; load-path and insert the following into your .emacs file:
...
;; 0.2 Made the package into a real minor mode, added a menu to the ;;    menu-bar, and made it customizable using customize.
...
;; 0.3a  - Major rewrites.
;;      - Added support for binding keys either global, local, or for ;;        a given mode.
;;      - Added support for saving and loading macros in several files. ;;      - Made the search for a possible collision with the key being ;;        bound.
;;      - Made managing of macros very easy using a context sensitive ;;        buffer with the description of all the macros.
;; 0.4a  - Major rewrites (again). This time due to realizing that ;;        there is no such thing as a buffer local key map, which is ;;        used together with major-mode map, minor-modes maps, and ;;        the global map.
;;      - Lots of code cleanup, this is the last release before a ;;        stable release is made.
;;      - Made it work with XEmacs (again).
;; 1.0  - Minor bug-fixes - and of course first stable release. ;; 1.1a1 - Answering with either RET or C-g to the question on
;;        which key to bind keyboard macros now makes power macros ;;        stop. This is for calling power-macros directly after ;;        having defined a macro.
;;      - Macros are now by default saved to .emacs. This also means ;;        that the files macros are saved to are edited rather than ;;        overwritten.
;;      - Removed pm-load as this function is not necessary anymore, ;;        simply used load-file instead.
;;      - Macros are now properly removed from files in case where ;;        the last macro is removed.
;;      - When power-macro-mode has been loaded, pm-define will now ;;        by default be invoked when a macro has been recorded. This ;;        may be changed with the variable
;;        `power-macros-run-on-end-macro'.
;;      - As a result of this new strategy,
;;        `power-macros-advice-end-kbd-macro' has been removed. ;;      - Cleaned up the descriptions, so they now follow the
;;        convention that it should be possible to read the first ;;        line of the description separately.
;;      - The keybinding for `pm-manage-macros' has been removed on ;;        request from Richard Stallman. Now you need to invoke it ;;        as M-x pm-manage-macros.
;;      - Pressing one of the power-macro bound keys in the manage ;;        buffer, now has the same effect on the description text in ;;        front of a field, as it has on the field itself.
;;      - The package is no longer a minor-mode. This is due to a ;;        request from Richard Stallman, who wants to include it with ;;        Emacs version 21. This means that you only need to load ;;        this package and then it will activate upon completing ;;        macro definition.
;;      - The package does no longer depend on the `cl' package. ;; 1.1a2 - Fixed a bug which made 1.1a1 break (when the function ;;        y-n-question wasn't defined).
...
;;      - pm-ask-for-mode now ask with simple y/n question on request ;;        from RMS.
;;      - It is now possible to continue defining a macro from the ;;        managing buffer.
;;        It does, however, require a patch for it to work in 
;;        Gnu Emacs 20.3.1
;;      - `pm-edit-field' now asks whether you want to copy the macro ;;        using y-or-n-p.
;;      - `pm-move-outside' now asks using `yes-or-no-p'.
;;      - Made the function pm-available-modes much simpler and much ;;        faster thanks to John Wiegley <johnw@gnu.org>.
;; 1.1a4 - pm-define now tells the user how to restart power-macros, when ;;        he has pressed RET to the question on which key to bind the ;;        macro.
;;      - Moved power-macros from customize group `Convenience' ;;        to `convenience'.
;;      - Added support in the manage buffer for macros not defined ;;        with power-macros.
;;      - The user will now be asked for a new key if he discovers that ;;        the key he was about to bind to (with pm-defin) is no good. ;;      - When asking for a key to bind to, the cursor will now be located ;;        in the minibuffer.
;;      - The user is now queryed for a name for the macro.
;;      - When the option `power-macros-verbose-on-load' is set, ;;        power-macros will tell which macros it loads.
;;      - Mode specific macro editing does now work (with regard to the ;;        names of the functions bound to mode specific keys).
;;      - Keys read now works with XEmacs.
;; 1.1a5 - A huge thanks to Rob van Vulpen <R.Vulpen@elsevier.nl>, who ;;        spell checked, grammer checked and much more on version 1.1a5. ;;      - It didn't work for XEmacs 20, since no (require 'edmacro) wasn't included, ;;        thanks to David Berthelot <d_berthelot@yahoo.com> for pointing this out for me. ;;      - Replaced edmacro-events-to-keys with events-to-keys for XEmacs. ;; 1.2  - Released.
...
- With power-macros you may give your macros a description, which   makes it easier to maintain them.
- With power-macros you may define macros as being global or 
  major-mode specific.
- With power-macros your macros are automatically saved to your .emacs   file for later sessions."
 
  :group 'convenience)
 
(defcustom power-macros-file user-init-file
  "File in which new macros are saved by default.
If you change the file name to something different from `.emacs' then you should insert the following line into your .emacs file:
...
(defcustom power-macros-save-on-change t
  "Non-nil means save macro to their files whenever a change is made. That is, whenever a change is made in the macro-manage buffer." 
  :type 'boolean
  :group 'power-macros)
 
(defcustom power-macros-run-on-end-macro t
  "Non-nil means that power-macros is executed upon macro definition. When disabled you may invoke power-macros (that is `pm-define' to be accurate) by pressing \\[pm-define]."
  :type 'boolean
  :group 'power-macros)
...
; TO-DO:
; - When moving a macro from one file to another when managing the ;  macros, it should be checked for possible collisions with a power ;  macro in the new file.
; - When the user ends editing a keyboard macro (with kbd-edit-kbdmacro) ;  it should be checked if the power macros should be saved.
...
(defvar pm-old-end-point nil
  "Original end of the description field being edited in the manage buffer. That is the location of the end before the editing started.") 
...
(substitute-key-definition 'end-kbd-macro 'pm-end-kbd-macro global-map) (substitute-key-definition 'start-kbd-macro 'pm-start-kbd-macro global-map) 
...
(defun pm-end-kbd-macro ()
  "End defining keyboard macros and call power-macros on the new macro."   (interactive)
  (end-kbd-macro)
...
(defun pm-define ()
  "Bind the latest defined keyboard macro to a key, and describe it. This will make Emacs ask for 
...
- a description for the macro."
  ...
  (let ((ask-again t))
    (while ask-again
      (let* ((cursor-in-echo-area t) ; Ensures that the cursor is in                                     ; the echo aread for the next question.             (key (pm-read-key-sequence "Bind macro to which key? (RET to quit) ")))         (if (member key '("\C-m" [return] "\C-g" [(control ?g)]))             (progn
              (message 
              (substitute-command-keys 
                "Macro not bound to any key. Press `\\[pm-define]' if you regret this."))               (setq ask-again nil))
 
          (let* ((mode (pm-ask-for-mode t))
                (file (if (y-or-n-p "Should the macro be saved for future sessions? ")                           (expand-file-name power-macros-file) ""))                 (existing-name (pm-get-macros key mode file))
            ...
            (when (catch 'pm-stop-def
              ...
              (pm-describe key mode file macro-name existing-name)               (setq ask-again nil))
          ))))))
 
 
(defun pm-describe (key mode file macro-name old-name)
  "This function sets up a buffer for describing a keyboard macro."   (let* ((desc (pm-get-desc key mode file))
    ...
    (local-set-key [(control c) (control c)] 
                  `(lambda () (interactive) 
                      (pm-end-description ,key ',mode ,file 
                                          ',macro-name ',old-name)))     (insert "Type a description of the macro below this line, and press C-c C-c\n")     (insert "------------------------------------------------------------------\n")     (if desc
        (insert desc))))
...
(defun pm-ask-for-mode (initial-definition)
  "Function used to ask about kind of macro (global or mode specific). Returns either `global' indicating that the macro should be a global macro, or a symbol representing the chosen mode, for example
`c++-major-mode'."
 
  (let* ((global-def 
          (not (if initial-definition
                  (y-or-n-p "Should the macro only be defined for the current major mode? ")                 (y-or-n-p "Should the macro only be defined for a single major mode? "))))) 
    ...
    (if global-def
      ...
      (if initial-definition
        ...
        ;; The mode is being changed from the manage-buffer, ask for the new mode.         (let ((mode (completing-read 
                    "Mode name: " (pm-available-modes)                     nil 1)))
          (if (string= mode "")
...
(defun pm-manage-macros ()
  "Bring up a manage buffer for editing properties of power macros."   (interactive)
...
- Press enter or the left mouse button on one of the fields to edit   them. When you are done editing the key, type or file field, you   are asked if your edits should form a new macro, or just update the   existing one.
...
- Sort the entries by pressing either `s' or `S' on top of the field   you wish to use for sorting.
- To edit one of the macros, simply press `e' on top of the macro. - Continue recording a macro by pressing `c'.
...
"))
  ...
  (let (key desc file macro-name mode p1 p2 tuple-start)
    ...
    (dolist (macro (pm-sort (pm-get-available-macros)))
      ...
      ;; Insert a number of spaces equal to the length of the delete       ;; text, to avoid that all the other fields moves, when a tuple       ;; is marked for deletion.
...
(define-derived-mode pm-macro-manage-mode fundamental-mode 
  ...
  (define-key pm-normal-edit-map [(return)] 'pm-edit-field-return)   (if (boundp 'xemacs-logo)
      (define-key pm-normal-edit-map [(button1)] 'pm-edit-field-mouse)     (define-key pm-normal-edit-map [(mouse-1)] 'pm-edit-field-mouse))   (define-key pm-normal-edit-map [(d)] 'pm-set-delete-mark)
  ...
  (define-key pm-normal-edit-map [(s)] (lambda () (interactive)                                         (pm-set-sort-func 'string<)))   (define-key pm-normal-edit-map [(S)] (lambda () (interactive)                                         (pm-set-sort-func 'pm-string>)))   (define-key pm-normal-edit-map [(c)] 'pm-continue-define-macro) 
 
  ;; ---------- Description edit map
  ;; Ordinary movement functions should still exists in the rest of   ;; the buffer when editing the description field. Therefore only   ;; inserting commands are disabled. This does however not disable   ;; commands like delete, return and backspace.
...
(defun pm-next-macro ()
  "Jumps to the key field of the next macro in the manage buffer."   (interactive)
  (pm-jump (lambda (x y) (< x y)) pm-token-list 'name)  
)
 
(defun pm-prev-macro ()
  "Jumps to the key field of the previous macro in the manage buffer."   (interactive)
  (pm-jump (lambda (x y) (> x y)) (reverse pm-token-list) 'name)  )
 
(defun pm-jump (dirp l &optional type)
  "Jumps to the next or previous field in the manage buffer. If the optional type is given, then the field must be of the given type."
  (let ((p (point))
        (found nil)
        (first-pos nil))
    (while l
      (when (and (not first-pos) (or (not type) (eq type (caar l))))         (setq first-pos (cadar l)))
      (if (and (funcall dirp p (cadar l)) (or (not type) 
                                              (eq type (caar l))))           (progn 
            (goto-char (cadar l))
            (setq l '())
            (setq found t))
        (setq l (cdr l))))
...
If the field is a description field, Emacs lets the user edit it in the manage buffer. Otherwise he is asked for a new value in the
mini-buffer.
    ...
    (t
      (progn
        ...
        (cond ((eq widget-type 'key)
              (setq key (pm-edit-key startField end mode file)))               ((eq widget-type 'type)
              (setq mode (pm-edit-type startField end key file)))               ((eq widget-type 'file)
...
(defun pm-edit-name (start end macro)
  "Change the name for macro at point in the manage buffer."   (let ((new-macro (pm-ask-for-macro-name macro)))
...
This function is invoked when enter is pressed on the `key' field of the manage buffer."
...
This function is invoked when enter is pressed on the `type' field of the manage buffer."
  (let ((mode (pm-ask-for-mode nil))
        text)
    (if (not (eq mode 'global))
            (setq text (format "mode - %s" (symbol-name mode)))       (setq text (symbol-name mode)))
 
    ;; Check if the new binding is ok.
    ;; (pm-possible-override nil key mode file)
    ;; This check has been disabled as it most often only will report a     ;; conflict with the macro itself.
...
This function is invoked when enter is pressed on the `file' field of the manage buffer."
  (let* ((old-name (buffer-substring start end))
        (file (pm-read-file-name "New file name: " 
                                  (file-name-directory old-name) nil)))     (pm-update-manage-buffer start end 
                            (if (string= file "") "none" file))     file))
 
(defun pm-read-file-name (prompt dir default)
  "Reads a filename from the mini-buffer and checks if it is a directory."   (let ((save (y-or-n-p "Should the macro be saved for future sessions? "))         (file-name-history pm-macro-files))
    (if save
        (let* ((f (read-file-name prompt dir default))
              (file (if (string= f "") f (expand-file-name f))))           (if (and file (file-directory-p file))
              (error "Filename is a name of a directory.")             file))
      "")))
...
This means that the whole buffer (except the region containing the description) is read-only.
 
This function is invoked when enter is pressed on the description field in the manage buffer."
 
  ;; Make the desc part writable.
  (setq buffer-read-only nil)
 
  ;; Go to start of region and record information about the text.   (goto-char start)
  ...
  (let ((keym (copy-keymap global-map))) ;; It is necesary to copy the map for                                         ;; things to work in XEmacs.     (define-key keym [(control c) (control c)] 
      (lambda () (interactive) (pm-end-edit-or-abort-desc-field 'end)))     (define-key keym [(control g)] 
      (lambda () (interactive) 
  ...
  ;; Make it impossible to insert at the beginning of the buffer.   (add-text-properties 1 2 '(front-sticky (read-only)))
 
  ;; Make it possible to insert at the first point in the description   ;; area.
  (add-text-properties (- start 1) start 
                      '(rear-nonsticky (face read-only)))
 
  ;; Now make everything else than the description area read-only.   ;; This is to ensure that the user has not bound any strange key,   ;; which is not disabled by the new key map.
  (make-face 'pm-face)
  (set-face-foreground 'pm-face "grey")
  (add-text-properties (point-min) start '(read-only t face pm-face))   (add-text-properties (- end 1) (point-max) 
...
When the user ends editing a description field in the manage buffer, then the manage buffer must be activated again, that is pressing enter on one of the fields should edit this field, etc."
  (setq inhibit-read-only t)
  (remove-text-properties (point-min) (point-max) 
                          '(read-only t local-map t keymap t face t))   (setq inhibit-read-only nil)
  ...
  (if (eq type 'abort)
      (progn
        ...
        (insert pm-old-desc "\n"))
    (progn
      (pm-update-info-lists pm-start-point (- pm-current-end-point                                               pm-old-end-point))       (set-text-properties pm-start-point (point) 
                          '(mouse-face highlight))
      (put pm-current-macro 'documentation 
          (buffer-substring-no-properties pm-start-point 
                                          pm-current-end-point))))   (setq pm-current-end-point nil)
  (use-local-map pm-normal-edit-map)
  (setq buffer-read-only t))
...
When the user is editing a description in the manage buffer, then he must complete these edits before proceeding with other edits. If he makes an edit outside the region containing the description, then he is queried whether he wants to discard his edits."
 
  (interactive)
 
  (let ((discard
        (yes-or-no-p "You have moved outside the description area, discard edits? ")))     (if discard
        (pm-end-edit-or-abort-desc-field 'abort)
      (progn
...
(defun pm-update-manage-buffer (start end text)
  "Inserts TEXT in place of content from START to END in the manage buffer. Furthermore it also makes the text highlight when mouse is over the given region."
...
(defun pm-delete-tuple-in-token-list (start end)
  "Removes tuples from `pm-token-list' in the region between START and END."   (let ((l pm-token-list) 
        elm t-start t-end)
    (setq pm-token-list '())
    (while l
      (setq elm (car l)
...
(defun pm-get-delete-pos ()
  "Finds position for \"delete\" for macro at point in manage buffer. That is, the position where the word \"delete\" may be inserted when the user presses `d' in the manage buffer."
...
(defun pm-get-tuple-info (f)
  "Returns information from the tuple at point in the manage buffer. The actual information is determined by the function F given as
...
(defun pm-set-delete-mark ()
  "Mark the macro at point, in the manage buffer, for deletion."   (interactive)
  (pm-set-or-unset-delete-mark 'set))
 
(defun pm-unset-delete-mark ()
  "Remove the deletion mark, for macro at point, in manage buffer."   (interactive)
  (pm-set-or-unset-delete-mark 'unset))
 
(defun pm-set-or-unset-delete-mark (type)
  "Set or unset the deletion mark for macro at point in the manage buffer."   (save-excursion
...
(defun pm-execute-deletion ()
  "Delete the macros marked for deletion in the manage buffer."   (interactive)
  (setq buffer-read-only nil)
  (save-excursion
    (let ((new-list '())
...
(defun pm-delete-macro (macro)
  "Deletes the macro named MACRO, and remove its key-bindings."   (pm-unset-key macro)
  (unintern macro))
 
;;}}}
;;{{{ Edit macro definition
 
;; The tricky part with the function below is that edit-kbd-macro does ;; not support a back-end to enter the edit buffer. Thus I need to go ;; trough the front-end, which requires that I answer a question in ;; the mini-buffer. Luckily this can be solved with a macro ;-)
 
(defun pm-edit-kbd-macro ()
  "Edits the keyboard macro at point in the manage buffer."   (interactive)
  (let* ((macro (pm-get-macro-at-point))
        (macro-str (symbol-name macro))
        (mode (get macro 'mode))
        (new-mac (vconcat [?\M-x] "edit-kbd-macro" [return ?\M-x]                           macro-str [return])))
 
    ;; I need to switch buffer to avoid that d,e,x have the meaning     ;; pm-delete-macro, pm-edit-kbd-macro etc. 
    (switch-to-buffer (get-buffer-create "*power-macro-dummy*"))     (funcall mode)
 
    (execute-kbd-macro new-mac)
    ;; Any idea how I can get pm-maybe-save executed when the user     ;; finishes editing the macro? The hook doesn't work when I enter     ;; the editing the above way.
    ))
...
(defun pm-continue-define-macro ()
  "Continue defining macro definition from manage macro buffer."   (interactive)
  (let* ((macro (pm-get-macro-at-point))
        (definition (copy-sequence (symbol-function macro))))
    (setq last-kbd-macro definition)
    (switch-to-buffer nil)
    (if (y-or-n-p "Continue macro definition in this buffer? ")         (progn
...
This is done based on the value of the variable `pm-sort-function'."   (sort macros pm-sort-function))
 
(defun pm-set-sort-func (direction)
  "Sorts the tuple in the manage buffer based on the field at point."   (let* ((elm (pm-get-context))
        (type (car elm)))
 
    (if (eq type 'text)
        (error "You must press `s' on top of a field.")       (if (eq type 'name)
          (setq pm-sort-function 'string<)
        (setq pm-sort-function
              `(lambda (x y) (,direction (format "%s" (get x ',type))                                         (format "%s" (get y ',type))))))))   (pm-manage-macros))
...
This function is invoked whenever a change is made in the manage buffer."
  ...
  (let (macros)
    (dolist (file pm-macro-files)
      (setq macros (pm-select `(lambda (name) 
                                (equal (get name 'file) ,file))                               (pm-get-available-macros)))
...
(defun pm-save-delete (file)
  "Delete all the invocations of `pm-def-macro' in the current buffer. Leave point at the location of the last call, or after the last
expression."
  (goto-char (point-min))
  (let ((last-pos (point-max))
    ...
    (while (setq sexp 
      ...
      (when (and (listp sexp) (eq (car sexp) 'pm-def-macro))
        ...
        (when (and (looking-at "$") (/= (point) (point-max)))           (delete-char 1)) 
        (when (and (looking-at "$") (/= (point) (point-max)))           (delete-char 1)) 
...
(defun pm-write-tuple-to-buffer (buf macro)
  ...
  (let* ((mode (get macro 'mode))
    ...
    ;; print header.
    (princ (concat "(;-" (pm-center ?- 66 " power-macros ") "-\n") buf)     (let ((text (concat (key-description key)
                        " - "
                        (if (eq mode 'global)
                            "global definition"
                          (concat "specific for " 
                                  (symbol-name mode))))))
      (princ (concat " ;-" (pm-center ?\ 66 text) "-\n") buf))     (princ (concat " ;" (make-string 68 ?-) "\n") buf) 
...
(defun pm-center (fill-char num text)
  "Returns TEXT with FILL-CHAR around it to center it on a line width NUM."   (let* ((mid (max (/ (- num (length text)) 2) 0))
...
;;    mode          - The name of the major mode for mode specific ;;                    macros or the symbol global.
 
(defun pm-get-macros (key mode file &optional all)
  "Returns the macro bound to KEY for mode MODE for file FILE. Any of the fields might be nil, which means that they are wild-cards. If ALL is t, then a list of matches is returned."
  (let ((list (pm-get-available-macros))
    ...
    (if all
        res
      (if (> (length res) 1)
          (error "Internal error: More that one tuple matched the query.")         (car res)))))
 
(defun pm-get-desc (key mode file)
  "Returns macro description for the given key/mode/file."   (let ((name (pm-get-macros key mode file)))
    (if (null name)
        ""
      (get name 'documentation))))
 
(defun pm-set-info (mode key desc file macro-name)
  "Inserts information into the power-macro database."   (put macro-name 'mode mode)
  (put macro-name 'key key)
  (put macro-name 'documentation desc)
  (put macro-name 'file file)
  (if (and (not (string= file "")) (not (member file pm-macro-files)))       (push file pm-macro-files)))
...
(defun pm-initialize-macro (macro)
  "Initialized a macro with the attributes required by power-macro."   (let ((key (get macro 'key))
        (desc (get macro 'documentation))
        (file (get macro 'file))
        mode map map-name)
      ...
      (let ((keys (where-is-internal macro global-map)))
        (if keys
          ...
          (dolist (mode-l (pm-available-modes))
            ...
            (setq map-name (intern (concat mode "-map")))             (setq map (if (boundp map-name)
                          (eval map-name)
                        nil))
            (when (keymapp map)
...
; The function pm-possible-override takes care of warning the user if ; he is about to overwrite a key or shadow key. Below the rules for ; checking this are described.
 
;--------------- meaning of symbols ---------------
; 
; (+) Means that the given test is implemented.
; (-) Means that the given test is not implemented. This is either ;    because I don't know how to do it, or because I find it very ;    unlikely that it will ever be used.
;
; (key, mode, file) in set pm-bind
;    Check if pm-macros contain a definition for the given
;    key,type,mode,file tuple.
;    A dash means that the given field does have a meaning (only used ;    for mode).
;    An asterisk is a wildcard meaning that this field should not be ;    included when searching for tuples.
 
; Key in set map.
;    Check if key is bound in the given map.
;
; Override.
;    This means that if the given check returns true, then the new ;    definition will override the given match(es).
;
; Shadows.
;    This means that if the given check returns true, then the new ;    definition will shadow the given match(es). Which means that the ;    existing definition(s) will not be visible.
;
; Invisible
;    This means that if the given check returns true, then the new ;    definition will have no effect. 
;
; Note: that the two clauses above might only be true for the given ; mode or the given buffer.
; 
; 1)
...
;--------------- Want to bind a mode specific key --------------- ; (+) 1)      Override  - (key,mode,file) in set PM-bind
...
; (-) 6 new)  Invisible - (key,active-minor-modes,*) in set PM-bind ; (+) 7 new)  Invisible - key in set active-minor-modes-maps
...
(defun pm-possible-override (new-def key mode file) 
  "Check whether the new key binding violates an existing one. For a list of all the different checks, please see the description in the source code."
 
  (let* ((buf (pm-create-buffer "*Warning*"))
        (any-output nil)
        main-macro macro-list macro)
    (setq pm-warn-buffer buf)
    (pm-insert "Your new definition might violate an existing definition. Below you see a report of what will happen if you make your binding.") 
 
 
    ;; First we need to attach power-macro information to every macro.     (dolist (macro (pm-get-available-macros))
      (pm-initialize-macro macro))
...
This definition will " (pm-outstand "replace") " an existing macro on the same key, with the same type and defined in the same file. Its description is:\n")           (pm-insert (pm-indent (get main-macro 'documentation))))) 
    ...
    (pm-insert "\n") ; This is to ensure that there is a location,                     ; which is not with the left-margin property. 
    ...
    (if any-output
        (save-excursion
          ...
          (setq buffer-read-only t)
          (goto-char (point-min))
          (if (not (yes-or-no-p "Continue defining the macro? "))               ;; Stop defining the macro.
              (progn
                (kill-buffer buf)
...
(defun pm-override-global (new-def key file main-macro)
  "Checks if the new global key binding violates an existing binding. See the source code for all the different checks."
...
(defun pm-override-mode (new-def key file mode main-macro)
  "Checks if the new mode key binding violates an existing bindings. See the source code for all the different checks."
...
(defun pm-check-binding (type key warn-type main-macro &optional mode)   "Verify if a key binding of type TYPE exists for KEY.
If this is the case then this function will insert a warning of type WARN-TYPE, telling that there is a problem. 
...
TYPE is one of: global, mode, minor-mode."
  ...
  (let* (map-name
        (func (cond ((eq type 'global) (pm-lookup-key global-map key))                     ((eq type 'mode)
                      (progn
                        (setq map-name (intern 
                                        (concat (symbol-name mode)                                                 "-map")))                         (if (boundp map-name)
                            (pm-lookup-key (eval map-name) key)
                          nil)))
                    ((eq type 'minor-mode) 
                      (pm-get-minor-mode-binding key))
                    (t (error "Internal error: Unknown type."))))         (where (symbol-name type)))
        ...
        (progn
          ...
          (if (eq warn-type 'conflict)
              (pm-insert "This definition will " 
                        (pm-outstand "conflict") " with ")             (if (eq warn-type 'invisible)
                (pm-insert "This definition will " 
                          (pm-outstand "be invisible") " because of ")               (if (eq warn-type 'shadows)
                  (pm-insert "This definition will " 
                            (pm-outstand "shadow") " for the ")               (error (format "Unknown type %s" warn-type))))) 
...
(defun pm-outstand (text)
  "Adds bold properties to the string given as argument."   (let ((new-text (concat text))) ;; We need to copy it to avoid adding                                   ;; properties to the original widget.     (add-text-properties 0 (length new-text) '(face bold) new-text)     new-text))
 
(defun pm-indent (text)
  "Returns TEXT but with the exception that it has indentation properties."   (let ((new-text (concat text))) ;; We need to copy it to avoid adding                                   ;; properties to the original widget.     (add-text-properties 0 (length new-text) '(left-margin 3) new-text)     new-text))
 
 
(defun pm-indent-regarding-to-properties ()
  "Indent the warning buffer with respect to the text property `left-margin'. This function runs through the buffer with warnings about overriding keys etc. and executes fill-region on the locations where the text property left-margin is set."
  (let ((cont t)
        (end (point-min))
        start)
    (while cont
      (setq cont nil)
      (setq start (next-single-property-change end 'left-margin))       (if start
...
(defun pm-lookup-key (map key)
  "Returns the function definition bound to key if it exists. If no definition is bound to the key, then nil is returned."   (let ((func (lookup-key map key)))
...
(defun pm-get-minor-mode-binding (key)
  "Returns the visible definitions of KEY in the active minor modes."   (let ((maps (current-minor-mode-maps))
        (binding nil))
    (while maps
...
(defun pm-unset-key (macro)
  ...
  (let ((key (get macro 'key))
    ...
    (if (eq mode 'global)
      ...
      (let* ((map (intern (concat (symbol-name mode) "-map"))))         (if (boundp map)
            (if (eq (lookup-key (eval map) key) macro)
                (define-key (eval map) key nil))
 
          ;; No map exists, we must then unadvice the function and run           ;; through all buffers.
          (if (fboundp mode)
  ...
  (let ((mode (get macro 'mode))
    ...
    (if (eq mode 'global)
      ...
      (let ((map (intern (concat (symbol-name mode) "-map"))))         (if (boundp map)
          ...
          ;; A mode-map didn't exists so now we need to make the           ;; binding much more manual.
...
(defun pm-set-or-unset-mode-key (macro mode key type)
  ...
  (let ((all-buffers (buffer-list))
    ...
    ;; Run through all the buffers and bind the key for buffers     ;; with the given major mode.
        ...
        (when (eq major-mode mode)
          (if (eq type 'set)
            ...
            (if (and (current-local-map)
                    (eq (lookup-key (current-local-map) key) macro))                 (local-unset-key key))))))
...
(defun pm-create-buffer (&optional name)
  "Switches to the named buffer or to the power-macros buffer. The buffer is created if it doesn't exist."
  (let* ((nm (if name name "*Power Macros Description*"))         (old-buf (get-buffer nm)))
    (if old-buf
        (kill-buffer old-buf))
...
(defun pm-select (func list)
  "Returns a list with elements from LIST, which matches the predicate FUNC."   (let ((res '()))
    (while list
...
(defun pm-available-modes ()
  ...
  (let ((mode-list (mapcar 'symbol-name
                          (apropos-internal "-mode\\'" 'fboundp))))     (mapcar 'list mode-list)))
...
(defun pm-new-dummy-macro-name ()
  ...
  (let ((num 1)
    ...
    (while (fboundp name)
      (setq num (+ num 1))
      (setq name (intern (format "pm-macro-%s" num))))     (symbol-name name)))
...
(defun pm-ask-for-macro-name (old-name)
  ...
  (let ((ask-again t)
      (name (intern 
              (if old-name (symbol-name old-name) (pm-new-dummy-macro-name)))))     (while ask-again
      (setq name
            (intern (read-from-minibuffer "A short unique name for the macro: "                                           (symbol-name name))))
